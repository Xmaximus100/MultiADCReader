...
% SIMPLE_PYSERIAL_NEW - MATLAB wersja programu do komunikacji szeregowej
% Przepisane z Pythona, zachowując całą funkcjonalność
...
% --- Zmienne globalne ---
global output_data repetitive_line data_collected data_amount
global frame_count_64 frame_count_32 last_frame_time
global timeout_ms extended_timeout_ms EXPECTED_FRAMES_64 EXPECTED_FRAMES_32 FRAME_TOLERANCE
global auto_send_enabled collection_stopped bytes_data_expected config_message_expected message_timeout
global session serial_port reader_timer
...
function [should_end, reason] = should_end_collection()
    ...
    global repetitive_line last_frame_time timeout_ms extended_timeout_ms
    global frame_count_64 frame_count_32 EXPECTED_FRAMES_64 EXPECTED_FRAMES_32 FRAME_TOLERANCE
...
function finish_collection()
    ...
    global output_data data_collected repetitive_line frame_count_64 frame_count_32 data_amount
    global last_frame_time auto_send_enabled FRAME_LEN FRAME_LEN_32
    global session serial_port
    ...
    global EXPECTED_FRAMES_64 FRAME_TOLERANCE
...
function collect_data(frame, frame_size)
    ...
    global output_data data_amount data_collected repetitive_line
    global frame_count_64 frame_count_32 last_frame_time collection_stopped
    global FRAME_LEN FRAME_LEN_32 auto_send_enabled serial_port
...
function process_frame(frame, frame_size)
    ...
    global data_amount output_data
...
function filename = export_to_dat(custom_filename)
    ...
    try
        ...
        fprintf(fid, "%% Exported data from serial session\n");
        fprintf(fid, "%% Columns: ch0 ch1 ch2 ch3 ch4 ch5 ch6 ch7\n");
        ...
        for i = 1:max_samples
            ...
            for ch = 0:7
                ...
                ch_data = output_data.(ch_name);
                if i <= length(ch_data)
                    row_values = [row_values, ch_data(i)];
                else
                    row_values = [row_values, NaN];
                end
            end
            % Formatuj wiersz z obsługą NaN
            row_str = "";
            for j = 1:length(row_values)
                if isnan(row_values(j))
                    if j == 1
                        row_str = "NaN";
                    else
                        row_str = row_str + " NaN";
                    ...
                    if j == 1
                        row_str = sprintf("%d", row_values(j));
                    else
                        row_str = row_str + sprintf(" %d", row_values(j));
                    end
                end
            end
            fprintf(fid, "%s\n", row_str);
...
function connect_serial(session)
    ...
    global serial_port reader_timer
...
function reader_timer_callback()
    ...
    global serial_port session message_timeout bytes_data_expected config_message_expected
    global collection_stopped data_collected last_frame_time
    FRAME_LEN = 64;
    FRAME_LEN_32 = 32;
...
function repl_loop(session)
    ...
    global timeout_ms extended_timeout_ms auto_send_enabled collection_stopped bytes_data_expected config_message_expected
    global output_data serial_port FRAME_LEN FRAME_LEN_32
    global BYTE_DATA_CMD STRING_DATA_CMD STRING_RESPONSE_CMD
